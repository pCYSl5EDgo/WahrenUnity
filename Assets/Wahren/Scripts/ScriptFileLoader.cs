                ...
                if (handle.IsValid())
                {
                    switch (handle.Status)
                    {
                        case ReadStatus.Complete:
                            ...
                            invalidCounter++;
                            deleteCommentJobsFinnalyDeallocated.Add(DeleteCommentJob.Schedule((TextFile*)scriptLoadReturnValue.Files.GetUnsafePtr() + i, isDebg));
                            break;
                ...
                if (handle.IsValid())
                {
                    switch (handle.Status)
                    {
                        case ReadStatus.Complete:
                            ...
                            deleteCommentJobs.Add(DeleteCommentJob.Schedule((TextFile*)scriptLoadReturnValue.Files.GetUnsafePtr() + i, isDebug));
                            break;
                        case ReadStatus.InProgress:
                            continue;
                        case ReadStatus.Failed:
